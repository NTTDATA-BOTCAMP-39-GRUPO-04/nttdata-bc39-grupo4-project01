...
import com.nttdata.bc39.grupo04.credit.util.Constants;
...
public class CreditServiceImpl implements CreditService {
	...
	public Mono<CreditDTO> makePaymentCredit(double amount, String creditNumber) {
		...
		if (amount < Constants.MIN_DEPOSIT_AMOUNT || amount > entity.getCreditAmount()) {
			throw new ServiceException(
					String.format(Locale.getDefault(), "Error, los limites de Pago son min: %d sol y max: %f sol",
							Constants.MIN_DEPOSIT_AMOUNT, entity.getCreditAmount()),
					HttpStatus.NOT_FOUND.toString(), LocalDate.now().toString(), HttpStatus.NOT_FOUND.value());
		}
	...
	public Mono<CreditDTO> makeChargeCredit(double amount, String creditCardNumber) {
		...
		if (amount < Constants.MIN_WITHDRAWAL_AMOUNT || amount > entity.getCreditAmount()) {
			throw new ServiceException(
	...
	private void validateCreateCredit(CreditDTO dto) {
		...
		if (!dto.getProductId().equals(Constants.CODE_PRODUCT_PERSON)
				&& !dto.getProductId().equals(Constants.CODE_PRODUCT_BUSINESS)
				&& !dto.getProductId().equals(Constants.CODE_PRODUCT_CARD_CREDIT)) {
			throw new ServiceException(
		...
		if (dto.getProductId().equals(Constants.CODE_PRODUCT_PERSON)) {
			CreditEntity credit = repository.findAll()
					.filter(x -> x.getProductId().equals(Constants.CODE_PRODUCT_PERSON)
		...
		if (dto.getProductId().equals(Constants.CODE_PRODUCT_CARD_CREDIT)) {
